...
handlers.user = function(data,callback){
    // choose specified method
     var chooseMethod = ['post','get','put','delete'];
     if(chooseMethod.indexOf(data.method) > -1){
...
//Container for User
 ...
 handlers._user.post = function(data,callback){
    ...
    var phone = typeof(data.payload.phone) == 'number' && data.payload.phone.toString().length == 10 ? data.payload.phone.toString() : false;
    ...
    if(firstName && lastname && phone && password && tosAgreement){
        ...
        _data.read('users',phone,function(err,data){
            if(err){
              ...
              _data.create('users',phone,user_object,function(err,data){
                 if(!err){
                     callback(200,data);
                 }else{
                     callback(400,{'error':'Data is not saved & Can not create user'});
                 }
              });
                    

            }else{
                callback(400,{'Error':'This user can not hashed password'});
            }

            }
            else{
                  callback(400,{'Error':'This Phone Number Exists'});
            }
           

        })
         }else{
        callback(400,{'Error':'Missing Fileds'});
    }
 ...
 // getting the phone number as required field
    ...
    if(phone){
     // if nothing is sent to update
      if(firstName || lastname || password){
        ...
        callback(400,{'Error':'No field for update'});
      }
